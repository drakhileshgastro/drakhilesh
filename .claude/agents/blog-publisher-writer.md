...
**Schema types to implement (Phase 3 — Supabase migration):**

- `MedicalWebPage` — from: `titleHi`, `metaDescription`, `category`, `slug`, `publishedAt`
- `Article` — from: `publishedAt`, `titleHi`, `titleEn`, `excerptEn`
- `Physician` (author) — hardcoded: Dr. Akhilesh Yadav, DM Gastroenterology, Orchid Medical Centre, HB Road, Ranchi

**⚠️ DO NOT implement FAQPage schema:**
Google retired FAQPage rich results site-wide on **May 7, 2026**. It no longer generates any rich results in Google Search. Do NOT inject `FAQPage` JSON-LD into any blog page — it is dead weight. The `faqs[]` array in blog-data.ts remains valuable for AEO/voice search, but the JSON-LD schema type has no SEO benefit.

**Valid schema types for medical content (as of 2026):**
`MedicalWebPage`, `Article`, `Physician`, `MedicalOrganization`, `LocalBusiness` — all still active.

**Action for Phase 3:** When migrating to Supabase, add `MedicalWebPage` + `Article` + `Physician` JSON-LD to `/blog/[slug]/page.tsx`. Skip FAQPage entirely.
